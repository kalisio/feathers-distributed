...
class RemoteService {
  constructor (app, options) {
    ...
    this.remote = true
    this.remoteOptions = options.remoteOptions
    if (options.methods) {
  ...
  async find (params) {
    debug('Requesting find() remote service on path ' + this.path + ' with key ' + this.key, params.query)
    try {
  ...
  async get (id, params) {
    debug('Requesting get() remote service on path ' + this.path + ' with key ' + this.key, id, params.query)
    try {
  ...
  async create (data, params) {
    debug('Requesting create() remote service on path ' + this.path + ' with key ' + this.key, data, params.query)
    try {
  ...
  async update (id, data, params) {
    debug('Requesting update() remote service on path ' + this.path + ' with key ' + this.key, id, data, params.query)
    try {
  ...
  async patch (id, data, params) {
    debug('Requesting patch() remote service on path ' + this.path + ' with key ' + this.key, id, data, params.query)
    try {
  ...
  async remove (id, params) {
    debug('Requesting remove() remote service on path ' + this.path + ' with key ' + this.key, id, params.query)
    try {
  ...
  generateCustomMethod (method) {
    ...
    return async function (data, params) {
      debug(`Requesting ${method}() remote service on path ` + this.path + ' with key ' + this.key, data, params.query)
      try {
