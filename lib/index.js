...
const getServicePath = (app, serviceDescriptor) => {
...
export function registerService (app, serviceDescriptor) {
  ...
  if (service) {
    if (service instanceof RemoteService) {
      debug('Already registered service as remote on path ' + servicePath + ' for app with uuid ' +
            app.uuid + ' and key ' + app.distributionKey)
    } else {
      debug('Already registered local service on path ' + servicePath + ' for app with uuid ' +
            app.uuid + ' and key ' + app.distributionKey)
    }
    return
  }
