...
export async function registerApplication (app, applicationDescriptor) {
  ...
  events.forEach(event => {
    app.serviceEventsSubscribers[key].on(event, object => {
      debug(`Dispatching ${event} remote service event on path ${object.path} for app with uuid ${app.uuid} and key ${app.distributionKey}`, object)
...
export function registerService (app, serviceDescriptor) {
  ...
  const events = []
  if (serviceDescriptor.events) serviceDescriptor.events.forEach(event => {
